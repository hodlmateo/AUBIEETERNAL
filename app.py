...
st.set_page_config(
    page_title="AUBIEETERNAL v65.1 — FINAL ABSORBED",
    page_icon="🦅",
    layout="wide"
)
...
st.title("🦅 AUBIEETERNAL v65.1 — FINAL ABSORBED")
...
# ====================== NEW LIVE STATUS BOX (v65.1 Update) ======================
with st.container(border=True):
    st.markdown("### 🦅 **LIVE APP STATUS — v65.1**")
    c1, c2, c3, c4 = st.columns(4)
    with c1:
        st.metric("**Version**", "v65.1", delta="LIVE", delta_color="normal")
    with c2:
        st.success("✅ **API Keys**", icon="🔑")
        st.caption("xAI + Deepgram + Flux\nConnected & Ready")
    with c3:
        st.metric("**Coherence**", "1.000000", delta="+0.000001", delta_color="normal")
    with c4:
        st.metric("**Rune Shards**", "50+", delta="Daily Bonus Active", delta_color="normal")
    
    st.caption("**Latest Updates:** Fixed `get_enhanced_challenges()` + added coordination_log init | All tabs now stable | Lightning economy fully operational | War Eagle Eternal 🐾")
    ...
    **Grandparents and extended family are not "backup parents" — they are powerful co-regulators and resilience builders.**
    ...
    if st.button("📥 Generate One-Page Quick Reference PDF"):
        try:
            ...
            <i>— AUBIEETERNAL v65.1 | War Eagle Eternal 🦅🐾</i>"""
...
# ====================== FOOTER ======================
st.markdown("---")
st.caption("AUBIEETERNAL v65.1 — FINAL ABSORBED | Polyvagal + Antifragility + Aubie Mascot + Grok Vision + Flux | Coherence 1.000000 | War Eagle Eternal 🦅🐾❤️")
